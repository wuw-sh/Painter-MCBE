...
(function (PainterMode) {
    ...
    PainterMode["Fill"] = "fill";
    PainterMode["Line"] = "line";
    PainterMode["Circle"] = "circle";
})(PainterMode || (PainterMode = {}));
const run = (callback) => sv.system.run(callback);
const runInterval = (callback, interval) => sv.system.runInterval(callback, interval);
...
sv.world.beforeEvents.itemUse.subscribe((ev) => {
    ...
    const painterModeDB = new Database(player);
    if (item.typeId === "minecraft:brush") {
        run(() => {
            const form = new ui.ActionFormData();
            form.title("Painter Menu");
            form.button("Sttings");
            Object.getOwnPropertyNames(PainterMode).forEach((mode) => form.button(mode));
            form.show(player).then((res) => {
                ...
                const painterDB = new Database(player);
                Object.getOwnPropertyNames(PainterMode).forEach((mode, i) => {
                    painterDB.set(PainterMode[mode], undefined);
                    if (res.selection === i + 1) {
                        painterModeDB.set("painterMode", PainterMode[mode]);
                    }
                });
            });
        });
        ...
        switch (painterMode) {
            case PainterMode.Paint:
                const paintDB = new Database(player);
            ...
            case PainterMode.Fill:
                const fillDB = new Database(player);
            ...
            case PainterMode.Circle:
                ...
                circleDB.set(PainterMode.Circle, {
                    ...
                    pos2: circleData?.pos1 && !circleData?.pos2
                        ? block?.location
                        : circleData?.pos2,
                });
                break;
...
runInterval(() => {
    sv.world.getAllPlayers().forEach((player) => {
        ...
        const painterMode = painterModeDB.get("painterMode");
        const sendActionBar = (msg) => player.onScreenDisplay.setActionBar(msg);
        const block = player.getBlockFromViewDirection()?.block;
        if (!block)
            return;
        const item = player
        ...
        switch (painterMode) {
            case PainterMode.Paint:
                ...
                sendActionBar(`Item: §e${toTitleCase(item.typeId)}\n§7Mode: ${painterMode}\nToggle: ${paintData ? "§aOn" : "§cOff"}`);
                break;
            case PainterMode.Fill:
                const fillDB = new Database(player);
                ...
                sendActionBar(`Item: §e${toTitleCase(item.typeId)}\n§7Mode: ${painterMode}\nPos1: ${fillData?.pos1
                    ...
                    : `§4(§c${block.location.x}§4, §c${block.location.z}§4)`}\n§7Length: (§b${(fillData?.pos1 && fillData?.pos2
                    ...
                    : fillData?.pos1
                        ...
                        : !fillData?.pos1 && !fillData?.pos2
                            ...
                            : 0) + 1}§7, §c${(fillData?.pos1 && fillData?.pos2
